import { Field, Section, useNodeUpdater } from './FormPrimitives'
    ...
    <div>
      ...
      {inputs.map((inp, i) => (
        <div key={i} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'flex-end' }}>
          <div style={{ flex: 1 }}>
            <Field label={i === 0 ? 'Item ID' : ''} value={inp.item_id} onChange={(v) => updateInput(i, { item_id: v })} placeholder="item-id" />
          </div>
          <div style={{ width: 60 }}>
      ...
      <Field label="Output item ID" value={d.output_item_id} onChange={(v) => update({ output_item_id: v })} placeholder="item-id" />
