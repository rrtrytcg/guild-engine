import { Field, TextArea, Toggle, Section, StatBlock, SearchableDropdown, useNodeUpdater } from './FormPrimitives'
...
export default function BossExpeditionInspector({ node }) {
  ...
  return (
    <div>
      ...
      <Section title="Loot" />
      <SearchableDropdown label="Kill loot table ID" value={d.loot_table_id} onChange={(v) => update({ loot_table_id: v })} typeFilter="loot_table" placeholder="Search loot tables" />
      <SearchableDropdown label="Fail loot table ID (optional)" value={d.fail_loot_table_id ?? ''} onChange={(v) => update({ fail_loot_table_id: v })} typeFilter="loot_table" placeholder="Search loot tables" />
